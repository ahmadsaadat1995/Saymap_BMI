<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PageSooratNazerPaper.aspx.cs" Inherits="NewMellat.Content.PageSooratNazerPaper" %>
...
<body>
    <form id="form1" runat="server">
       <div align="center">
        <table align="center" cellpadding="0" cellspacing="0">
            <tr>
                <td>
                     <dx:ASPxComboBox ID="ASPxComboBox2" runat="server" AnimationType="Fade" EnableTheming="True" RightToLeft="True" SelectedIndex="0" Theme="SoftOrange">
                        <Items>
                            <dx:ListEditItem Selected="True" Text="پرداخت بدون جریمه" Value="0" />
                            <dx:ListEditItem Text="پرداخت با جریمه" Value="1" />
                            
                        </Items>
                    </dx:ASPxComboBox>
                   </td>
                <td class="RightColumn">
                   <asp:Label ID="Label7" runat="server" Text=":جریمه"></asp:Label></td>
                <td align="right">
                    <dx:ASPxComboBox ID="ASPxComboBox1" runat="server" AnimationType="Fade" EnableTheming="True" RightToLeft="True" SelectedIndex="0" Theme="SoftOrange">
                        <Items>
                            <dx:ListEditItem Selected="True" Text="تایید" Value="1" />
                            <dx:ListEditItem Text="عدم تایید" Value="2" />
                             <dx:ListEditItem Text="بدون پاسخ" Value="3" />
                        </Items>
                        <ClientSideEvents SelectedIndexChanged="function(s, e) { OnAnswerChanged(s,e); }" /> 
                    </dx:ASPxComboBox>
                </td>
                <td class="RightColumn">
                    <asp:Label ID="Label6" runat="server" Text=":اعلام نظر"></asp:Label>
                </td>
            </tr>
            <tr>
                <td>
        ...
        <dx:ASPxGridView ID="ASPxGridView1" runat="server" Theme="SoftOrange" AutoGenerateColumns="False" DataSourceID="sourcePages" KeyFieldName="paId" RightToLeft="True" Width="100%">
            ...
            <Columns>
                ...
                <dx:GridViewDataComboBoxColumn Caption="وضعیت" FieldName="paEdare" VisibleIndex="1">
            ...
            <asp:SqlDataSource ID="sourcePages" runat="server" ConnectionString="<%$ ConnectionStrings:NewMellatConnectionString %>" DeleteCommand="UPDATE tbProjectPaper SET paAct = 0 WHERE (paId = @paId)" 
                InsertCommand="INSERT INTO tbProjectPaper(paShenase, paNumber, paDate, paProject, paAct, paType, paBody, paEdare,paDelay) VALUES (@paShenase, @paNumber, @paDate, @paProject, 1, @paType, @paBody, @paEdare,@paDelay)" 
                SelectCommand="SELECT paId, paShenase, paNumber,  paProject, paAct, paType, paBody,DimDate.PersianStr as paDate,  
                                        paEdare=case paEdare when 1 then N'تایید' when 2 then N'عدم تایید' when 3 then N'بدون پاسخ' end,
                                        paDelay=case paDelay when 0 then N'پرداخت بدون جریمه' when 1 then N'پرداخت با جریمه' end
                                FROM tbProjectPaper Left  Join DimDate On DimDate.GregorianDate = tbProjectPaper.paDate
                                WHERE (paAct = 1) AND (paType = @paType) AND (paProject = @paProject)" 
                UpdateCommand="UPDATE tbProjectPaper SET paShenase = @paShenase, paNumber = @paNumber, paBody = @paBody, paEdare = @paEdare, paDelay = @paDelay WHERE (paId = @paId)">
                <DeleteParameters>
                ...
                <InsertParameters>
                    ...
                    <asp:ControlParameter ControlID="ASPxComboBox1" Name="paEdare" PropertyName="Value" />
                    <asp:ControlParameter ControlID="ASPxComboBox2" Name="paDelay" PropertyName="Value" />
                </InsertParameters>
                <SelectParameters>
                    <asp:QueryStringParameter Name="paType" QueryStringField="type" />
    ...
    <script>
        function OnAnswerChanged(s, e) {
            if ($('#ASPxComboBox1_I').val() == "عدم تایید") {
                $('#ASPxComboBox2').hide();
                $('#Label7').hide();
            }
            else {
