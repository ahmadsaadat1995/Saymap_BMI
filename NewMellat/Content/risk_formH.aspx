<%@ Page Title="" Language="C#" MasterPageFile="~/Site2.Master" AutoEventWireup="true" CodeBehind="risk_formH.aspx.cs" Inherits="NewMellat.Content.risk_formH" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
     <script type="text/javascript">
         function loadDoc7(parent) {
             var oWnd = radopen("PageAttach.aspx?Parent=" + parent + "&Type=28&tyExt=22", "RadWindow1");
             oWnd.setSize(910, 500);
             oWnd.Center();
         }
         function loadDoc2(parent) {
             var oWnd = radopen("riskInsertLetter.aspx?project=" + parent + "&Type=8", "RadWindow2");
        ...
        <dx:ASPxGridView ID="ASPxGridView1" runat="server" Theme="Office2010Silver" AutoGenerateColumns="False" DataSourceID="source1" 
            ...
            <Columns>             
                ...
                <dx:GridViewDataTextColumn Caption="تاریخ نامه" FieldName="letterDate" VisibleIndex="8">
                 ...
                 <dx:GridViewDataTextColumn Caption="مستندات بارگزاری نشده" FieldName="docNotEnter" VisibleIndex="9">
                    <Settings AutoFilterCondition="Contains" />
                    <EditFormSettings Visible="False" />
                </dx:GridViewDataTextColumn>
                 ...
                 <dx:GridViewDataTextColumn Caption="نامه " VisibleIndex="11">
                    <DataItemTemplate>
          ...
          <dx:ASPxGridViewExporter ID="ASPxGridViewExporter1" runat="server" FileName="تهیه گزارش" GridViewID="ASPxGridView1">
     ...
     <asp:SqlDataSource ID="source1" runat="server" ConnectionString="<%$ ConnectionStrings:NewMellatConnectionString %>" SelectCommand="
         select r.*,g.ghCode ,prTitle,prPeymankar,TechnicalSuvisor,prVahed,pp.paShenase as letterShenase ,pp.paNumber as letterNo,d0.PersianStr as letterDate,
         d1.PersianStr as deadline,docNotEnter
from risk_form1 as r left join tbProjects as p on p.prId=r.projectId
					 left join tbGharardad as g on p.prId=g.ghProject
					 left join (select * from tbProjectPaper where paType=201)as pp on pp.paProject=p.prId
					 Left Outer Join DimDate as d0 On d0.GregorianDate = pp.paDate
					 Left Outer Join DimDate as d1 On d1.GregorianDate = DATEADD(day, 7,pp.paDate)
where [status]=8      and ((prVahed IN (SELECT prProject  FROM tbAdminProjects WHERE (prType = 3)  AND (prCode = @prCode)  )) 
