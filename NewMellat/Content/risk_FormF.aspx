<%@ Page Title="" Language="C#" MasterPageFile="~/Site2.Master" AutoEventWireup="true" CodeBehind="risk_FormF.aspx.cs" Inherits="NewMellat.Content.risk_FormF" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <script type="text/javascript">
      function loadDoc7(parent) {
          var oWnd = radopen("PageAttach.aspx?Parent=" + parent + "&Type=27&tyExt=26,25,24,23,22,21", "RadWindow1");
          oWnd.setSize(910, 500);
          oWnd.Center();
      }
      function loadDoc2(parent) {
          var oWnd = radopen("riskInsertLetter.aspx?project=" + parent + "&Type=7", "RadWindow2");
     ...
     <div class="box box-danger">
        <dx:ASPxGridView ID="ASPxGridView1" runat="server" Theme="Office2010Silver" AutoGenerateColumns="False" DataSourceID="source1" 
          ...
          <dx:ASPxGridViewExporter ID="ASPxGridViewExporter1" runat="server" FileName="ارسال RTP " GridViewID="ASPxGridView1">
...
from risk_form1 as r left join tbProjects as p on p.prId=r.projectId
					 ...
					 left join (select * from tbProjectPaper where paType=206)as pp on pp.paProject=p.prId
					 Left Outer Join DimDate as d0 On d0.GregorianDate = pp.paDate
					 Left Outer Join DimDate as d1 On d1.GregorianDate = DATEADD(day, 14,pp.paDate)
where [status]=6      and ((prVahed IN (SELECT prProject  FROM tbAdminProjects WHERE (prType = 3)  AND (prCode = @prCode)  )) 
